    ...
    return (
        <Modal centered show={showForkIndexerModal} onHide={() => setShowForkIndexerModal(false)} className="bg-gray-50">
            ...
            <Modal.Body className="p-4">
                ...
                {error && (
                    <Alert variant="danger" className="px-4 py-2 mt-3 font-semibold text-red-700 text-sm text-center border border-red-300 bg-red-50 rounded-lg shadow-md">
                        {error}
                    </Alert>
                )}
            </Modal.Body>
            <Modal.Footer className="border-t border-gray-300">
                <Button variant="secondary" onClick={() => setShowForkIndexerModal(false)} className="bg-gray-500 text-white hover:bg-gray-600">
                    Cancel
                </Button>
