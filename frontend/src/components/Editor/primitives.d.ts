...
function _mergeNamespaces(n, m): any {
...
interface ExecutionOutcomeWithReceipt {
  executionOutcome: {
    blockHash: string;
    id: string;
    outcome: {
...
interface IndexerTransactionWithOutcome {
...
type ActionView =
  ...
  | DeleteAccountAction
  | DelegateAction;
interface StateChangeWithCauseView {
  change: {
    accountId: string;
    keyBase64: string;
    valueBase64: string;
...
interface TransactionProcessingCause {
  txHash: string;
}
interface ActionReceiptProcessingStartedCause {
  receiptHash: string;
}
interface ActionReceiptGasRewardCause {
  receiptHash: string;
}
interface ReceiptProcessingCause {
  receiptHash: string;
}
interface PostponedReceiptCause {
  receiptHash: string;
}
...
type StateChangeValue =
  ...
  | AccessKeyDeletionValue
  | DataUpdateValue
  | DataDeletionValue
  | ContractCodeUpdateValue
...
declare class Block {
  ...
  private readonly executedReceipts;
  /**
  ...
  private readonly buildActionsHashmap;
  private readonly buildEventsHashmap;
