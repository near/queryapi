...
describe('Editor for creating a new indexer', () => {
  beforeEach(() => {
    render(
      <Editor
        ...
        actionbuttontext="save / register"
        onLoadErrorText="An error occured while trying to query indexer function details from registry."
        queryIndexerFunctionDetails={jest.fn()}
      />,
    );
  });
  ...
  test('verifies the visibility and functionality of buttons: Reset, Format Code, and Register Function', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText('Oh snap! We could not format your code. Make sure it is proper Javascript code.'),
