...
const LatestBlock: React.FC<LatestBlockProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const updateFinalBlock = async () => {
            try {
                const res = await rpcBlock("final");

                if (res.result && res.result.header && res.result.header.height) {
        ...
        updateFinalBlock();
        const intervalId = setInterval(updateFinalBlock, 1000);
    ...
    return (
        <div>
            {latestFinalBlock !== null && props.indexerBlockHeight !== undefined ?
                ...
                `Indexer is not yet synced Latest Final Block Height: ${latestFinalBlock}`}
