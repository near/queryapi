...
function writeEliasGammaBits(x, result, startBit, writeZeros = false) {
    if (x === 0)
        return { nextBit: startBit, result };
    if (x === 1) {
...
function decompressToBitmapArray(compressedBytes) {
    ...
    while (compressedBitIdx < compressedBitLength) {
        ...
        const { x, lastBit } = decodeEliasGammaEntryFromBytes(compressedBytes, compressedBitIdx);
...
function addIndexCompressedLast(compressedBase64, index, lastEliasGammaStartBit, maxIndex) {
    ...
    const { x, lastBit } = decodeEliasGammaEntryFromBytes(originalCompressed, lastEliasGammaStartBit);
    ...
    if (index - maxIndex === 1) {
        ...
        throw Error(`addIndexCompressedLast: cannot write into ${index} before ${maxIndex}`);
    }
...
function addIndexCompressedFull(compressedBase64, index) {
    ...
    const { result: compressed, lastEliasGammaStartBit, maxIndex, } = compressBitmapArray(newBitmap);
    return {
...
function addIndexCompressed(compressedBase64, index, lastEliasGammaStartBit, maxIndex = -1) {
    if (maxIndex < 0 || index <= maxIndex) {
        return addIndexCompressedFull(compressedBase64, index);
    }
    else {
        return addIndexCompressedLast(compressedBase64, index, lastEliasGammaStartBit, maxIndex);
    }
}
module.exports = {
    addIndexCompressed,
