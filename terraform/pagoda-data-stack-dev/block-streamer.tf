module "queryapi_block_streamer_mainnet_container" {
  ...
  container = {
    args  = []
    image = "us-central1-docker.pkg.dev/pagoda-data-stack-dev/queryapi/queryapi-block-streamer:latest"
    ...
    env = [
      ...
      },
      {
        ...
        value = "https://queryapi-hasura-graphql-mainnet-24ktefolwq-ew.a.run.app/v1/graphql" // prod hasura
      }
    ]
...
resource "google_compute_instance" "queryapi_block_streamer_mainnet" {
  ...
  machine_type              = "n2-custom-10-55296"
  zone                      = "europe-west1-b"
