...
declare const process: {
  env: HasuraClientConfig;
};

export default class HasuraClient {
  constructor() {}

  async executeSql(sql: string, opts: ExecuteSqlOpts) {
  ...
  async executeMetadataRequest(type: string, args: any) {
    const response = await fetch(`${process.env.HASURA_ENDPOINT}/v1/metadata`, {
      method: "POST",
      headers: {
        "X-Hasura-Admin-Secret": process.env.HASURA_ADMIN_SECRET,
      },
  ...
  async executeBulkMetadataRequest(metadataRequests: any) {
    return this.executeMetadataRequest("bulk", metadataRequests);
  }

  async isSchemaCreated(schemaName: string) {
  ...
  createSchema(schemaName: string) {
    return this.executeSql(`CREATE schema ${schemaName}`, { readOnly: false });
  }

  runMigrations(schemaName: string, migration: string) {
  ...
  async getTableNames(schemaName: string) {
    const { result } = await this.executeSql(
  ...
  async trackTables(schemaName: string, tableNames: string[]) {
  ...
  async trackForeignKeyRelationships(schemaName: string) {
    const foreignKeys: ForeignKey[] = await this.getForeignKeys(schemaName);
        ...
        .map((foreignKey) => [
          ...
          },
          {
            ...
            args: {
              ...
              using: {
                foreign_key_constraint_on: Object.keys(
                  foreignKey.column_mapping
                )[0],
              },
            },
          },
