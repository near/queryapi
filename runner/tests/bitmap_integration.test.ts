...
function getBlockFromJSON (blockHeight: number): Block {
  const paddedBlockHeight = blockHeight.toString().padStart(11, '0');
  const streamerMessage: StreamerMessage = JSON.parse(fs.readFileSync(`tests/blocks/${paddedBlockHeight}/streamer_message.json`, 'utf-8'));
...
describe('Indexer integration', () => {
  ...
  it('works', async () => {
    const indexerCode = fs.readFileSync('tests/bitmap_indexer_logic.js', 'utf-8');
    const blocksIndexerQuery = gql`
      query {
        morgs_near_actions_index {
          height
        }
      }
    `;
    const schema = fs.readFileSync('tests/bitmap_indexer_schema.sql', 'utf-8');
    ...
    const blockHeight = 115598802;

    await indexer.execute(getBlockFromJSON(blockHeight));
    ...
    expect(blocks.map(({ height }: any) => height)).toEqual([blockHeight]);
  });
  ...
  it('test context db', async () => {
    ...
    await indexer.execute(getBlockFromJSON(115185109));
