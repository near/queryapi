...
export const METADATA_TABLE_UPSERT = 'INSERT INTO %I._metadata (attribute, value) VALUES %L ON CONFLICT (attribute) DO UPDATE SET value = EXCLUDED.value RETURNING *';
...
export default class IndexerMeta {
  ...
  private readonly logInsertQueryTemplate: string = 'INSERT INTO %I._logs (block_height, date, timestamp, type, level, message) VALUES %L';
  ...
  async writeLogs (
    ...
    }, `Failed to insert ${entriesArray.length > 1 ? 'logs' : 'log'} into the ${this.indexerConfig.schemaName()}._logs table`)
      .finally(() => {
        writeLogSpan.end();
      });
  }
