...
`;
  ...
  test('GetTables works for a variety of input schemas', async () => {
    ...
    const simpleSchemaTables = indexer.getTableAndColumnNames(SIMPLE_SCHEMA);
    expect(Array.from(simpleSchemaTables.keys())).toStrictEqual(['"posts"']);

    const socialSchemaTables = indexer.getTableAndColumnNames(SOCIAL_SCHEMA);
    expect(Array.from(socialSchemaTables.keys())).toStrictEqual(['"posts"', '"comments"', '"post_likes"']);

    const stressTestSchemaTables = indexer.getTableAndColumnNames(STRESS_TEST_SCHEMA);
    expect(Array.from(stressTestSchemaTables.keys())).toStrictEqual([
      'creator_quest',
      'composer_quest',
      '"contractor - quest"',
      '"posts"',
    ...
    expect(() => {
      indexer.getTableAndColumnNames(duplicateTableSchema);
  ...
  test('GetTables returns table names with quotes if quoted', async () => {
    ...
    CREATE TABLE
      "postsTable" (
        ...
        "content" TEXT NOT NULL,
        "BlockTimestamp" DECIMAL(20, 0) NOT NULL,
        "accounts_liked" JSONB NOT NULL DEFAULT '[]',
        lastComment_timestamp DECIMAL(20, 0),
        CONSTRAINT "posts_pkey" PRIMARY KEY ("id")
      );
    ...
    CREATE INDEX "posts_last_comment_timestamp_idx" ON "posts" ("last_comment_timestamp" DESC);
