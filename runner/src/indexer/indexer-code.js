...
export async function getBlock (block, context) {
  ...
  function decompressToBitmapArray (compressedBytes) {
    ...
    const bufferLength = Math.ceil(resultBitIdx / 8);
  ...
  allReceivers = Array.from(
    allReceivers.reduce((totalReceivers, receiver) => {
