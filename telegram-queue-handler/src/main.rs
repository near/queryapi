use aws_lambda_events::event::sqs::{SqsEvent, SqsMessage};use lambda_runtime::{run, service_fn, Error, LambdaEvent};use shared::{BorshDeserialize, types::primitives::{AlertDeliveryTask}};use teloxide::{payloads::SendMessageSetters, requests::{Request, Requester}};
...
async fn function_handler(event: LambdaEvent<SqsEvent>) -> Result<Vec<()>, QueueError> {
    ...
    let token = std::env::var("TELEGRAM_TOKEN")
        .expect("TELEGRAM_TOKEN must be set for the lambda");
...
SELECT name FROM alert_rules WHERE id = $1
        ...
        if let shared::types::primitives::DestinationConfig::Telegram { chat_id, destination_id } = delivery_task.destination_config {

            let (status, response) = match bot.send_message(
                chat_id.to_string(),
                format!(
